...
def _content_blob(content: Dict[str, Any]) -> str:
    """Flatten packaged content to a searchable string."""
    return _json.dumps(content, ensure_ascii=False)


def _line_represented(line_text: str, blob: str) -> bool:
    """
    A line is represented if its substantial alphanumeric tokens survived into
    the packaged content. Lines made only of symbols/separators are treated as
    represented (they carry no recoverable data). This is content-based, not
    range-based, so a line a parser silently dropped is NOT counted as covered.
    """
    tokens = [t for t in re.findall(r"[A-Za-z0-9_]+", line_text) if len(t) >= 3]
    if not tokens:
        return True
...
def _detect_gaps(
    ...
    """
    Find non-empty lines whose content did NOT survive into the packaged output.

    Coverage is verified by CONTENT PRESENCE, not line-index range: a line a
    parser discarded (e.g. a table label row that produced too few cells) is
    detected here even though it sits inside a block's index span. Such lines
    are returned for explicit prose fallback + flagging so no physical line
    ...
    for l in line_set:
        if l["is_empty"]:
            continue
        text = _line_text(l)
        if not _line_represented(text, blob):
            gaps.append({"line_index": l["line_index"], "text": text})
    return gaps
...
def _package_mixed(
    ...
    # Gaps detected against the assembled sub_blocks (content presence): any
    # segment line not represented in any sub_block survives as a gap.
    gaps = _detect_gaps(line_set, {"sub_blocks": sub_blocks})
...
def _package_from_blocks(
    ...
    if structure_type == "prose":
        # Prose carries the full segment text verbatim — nothing can be dropped.
        return {"text": text}, []

    if structure_type == "kv_block":
        ...
        # mixed computes its own content-based gaps over assembled sub_blocks.
        return _package_mixed(blocks, line_set, classification)
    else:
        return {"text": text}, []

    gaps = _detect_gaps(line_set, content)
    return content, gaps
...
def _hr_table(content: Dict[str, Any]) -> str:
    ...
    if headers:
        out.append("  ".join(str(h) if h is not None else "null" for h in headers))
...
def _hr_mixed(content: Dict[str, Any]) -> str:
    ...
    for sb in content.get("sub_blocks", []):
        ...
        # context sub-blocks are intentionally NOT rendered here: their raw
        # lines overlap the structured sub-blocks (e.g. the same key=value line
        # the kv_block already renders), so emitting them duplicates content in
        # the human-readable view. The context lines remain in machine_readable.
