...
# ---------------------------------
# Glyph-aware depth detection
# ---------------------------------

GLYPH_DEPTH_RE = re.compile(
    r"^(\s*)((?:[│|]\s*)*)(?:[├└+]\s*[-─]+\s*)?"
)
...
def _glyph_aware_depth(line_text: str, unit: int) -> int:
    """
    Compute effective depth from both leading spaces and
    tree glyph continuation characters.
    Handles: +-- style, ├── style, |   continuation.
    Falls back to space-only depth if no glyph match.
    """
    match = GLYPH_DEPTH_RE.match(line_text)
    if not match:
        return 0
    leading_spaces = len(match.group(1))
    pipe_chars     = len(match.group(2).replace(" ", ""))
    return (leading_spaces // max(unit, 2)) + pipe_chars
...
def _build_tree(
    ...
    for line in lines:
        depth = _glyph_aware_depth(line["text"], unit)
